...
dojo.declare("dojox.xmpp.PresenceService", null, {
	...
	publish: function(presence){
		//// 0 && console.log("Presence::publish() ", presence);
	...
	*/
	...
	_setPresence: function() {
			...
			if(this.isInvisible) {
				// 0 && console.log("was invisible, making visible");
				this._setVisible();
			}
	...
	*/
	...
	toggleBlockContact: function(jid) {
		...
		// 0 && console.log("setting outbound block for ", jid, this.restrictedContactjids[jid]);
	...
	toggleContactInvisiblity: function(jid) {
		...
		// 0 && console.log("setting outbound presence for ", jid, this.restrictedContactjids[jid]);
		this._updateRestricted();
		return this.restrictedContactjids;
	},
	...
	_updateRestricted: function() {
		...
		for(var jid in this.restrictedContactjids) {
			...
			// 0 && console.log("restricted ", jid, item);
			...
			req.append("</query>");
		req.append("</iq>");
		// 0 && console.log("Restricted list: ", req.toString());
			...
			req2.append("</iq>");
				...
				// 0 && console.log("Activate list: ", req2.toString());
	...
	_setVisible: function() {
			...
			var req = new dojox.string.Builder(dojox.xmpp.util.createElement("iq",props,false));
				...
				req.append("</query>");
			req.append("</iq>");
			// 0 && console.log(req.toString());
			this.session.dispatchPacket(req.toString());
	},
	
	_setInvisible: function() {
		// 0 && console.log("Setting user as invisible");
		var props={
			id: this.session.getNextIqId(),
			from: this.session.jid + "/" + this.session.resource,
			type: "set"
