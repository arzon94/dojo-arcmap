...
function(dojo, defaults, registry, keys, Mouse, Canvas, Undo, Anchors, Stencil, StencilUI, utilCommon){
	return dojo.declare("dojox.drawing.Drawing", [], {
		...
		_createCanvas: function(){
			 0 && console.info("drawing create canvas...");
		...
		startup: function(){
			// 0 && console.info("drawing startup")
		},
		...
		initPlugins: function(){
			...
			dojo.forEach(this.plugins, function(p, i){
				...
				// 0 && console.log('drawing.plugin:::', p.name, props)
		...
		exporter: function(){
			...
			//		Array of objects.
			 0 && console.log("this.stencils", this.stencils);
		...
		changeDefaults: function(/*Object*/newStyle,/*Boolean*/value){
			...
			// 0 && console.log("----->>> changeDefault: ",newStyle, " value?: ",value);
			if(value!=undefined && value){
				...
				for(var nm in newStyle){
					for(var n in newStyle[nm]){
						// 0 && console.log("  copy", nm, n, " to: ", newStyle[nm][n]);
		...
		onRenderStencil: function(/* Object */stencil){
			...
			// 0 && console.info("--------------------------------------dojox.drawing.onRenderStencil:", stencil.id);
		...
		setTool: function(/* String */type){
			...
			// 0 && console.log("new tool arg:", type, "curr:", this.currentType, "mode:", this.mode, "tools:", this.tools)
			...
			try{
				this.currentStencil = new this.tools[this.currentType]({container:this.canvas.surface.createGroup(), util:utilCommon, mouse:this.mouse, keys:keys});
				 0 && console.log("new tool is:", this.currentStencil.id, this.currentStencil);
		...
		set: function(name, value){
			...
			//		widget's api.
			 0 && console.info("Attempting to set ",name," to: ",value,". Set currently not fully supported in Drawing");
		},
