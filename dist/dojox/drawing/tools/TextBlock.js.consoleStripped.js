...
function(dojo, dijit, oo, registry, StencilText){
	...
	dojo.addOnLoad(function(){
		...
		//  0 && console.log("Removing conedit");
	...
	var TextBlock = oo.declare(
		...
		function(options){
			...
			// 0 && console.log("TextBlock:", this.id)
		},
		{
			// summary:
...
=====*/
			...
			connectTextField: function(){
				...
				kc2 = dojo.connect(conEdit, "keydown", this, function(evt){
					...
					if(evt.keyCode==220){
						if(!greekHelp){
							 0 && console.info("For greek letter assistance instantiate: dojox.drawing.plugins.drawing.GreekPalette");
							return;
						}
						dojo.stopEvent(evt);
			...
			execText: function(){
				...
				this.editMode = false;
				 ...
				 0 && console.log("EXEC TEXT::::", this._postRenderCon);
			...
			edit: function(){
				...
				var text = this.getText() || "";
				 0 && console.log("EDIT TEXT:",text, " ",text.replace("/n", " "));
				// NOTE: no mouse obj
			...
			getSelection: function(node){
				...
				if(dojo.doc.selection){
					...
					this._caretEnd = dojo.global.getSelection().getRangeAt(node).endOffset;
					 0 && console.log("Caret start: ", this._caretStart," end: ", this._caretEnd);
				}
			},
				...
				if(dojo.doc.selection){ // IE
					...
					switch(what){
						...
						case "stored":
							...
							// 0 && console.log("start: ",this._caretStart, " end: ",this._caretEnd," dif: ",dif);
					...
					switch(what){
						case "end":
							 0 && console.log("len:", nodes[nodes.length - 1].textContent.length);
						...
						case "all":
							...
							break;
						case "stored":
							 0 && console.log("Caret start: ",this._caretStart," caret end: ",this._caretEnd);
							r.setStart(nodes[0], this._caretStart);
							r.setEnd(nodes[0], this._caretEnd);
					}
					selection.addRange(r);
					 0 && console.log("sel ", what, " on ", node);
				}
			}
		}
	);
