...
var TableHandler = declare(_Plugin, {
	...
	getTableInfo: function(forceNewData){
		...
		if(this.tableData){
			...
			// 0 && console.log("returning current tableData:", this.tableData);
		...
		// 0 && console.log("td:", td); 0 && console.log("tr:", tr); 0 && console.log("tbl:", tbl)
	...
	connectDraggable: function(){
		...
		//
		// 0 && console.log("connectDraggable", tbl);
		...
		tbl.ondragstart = function(){
			// 0 && console.log("ondragstart");
		};
		tbl.ondrag = function(){
			alert("drag")
			// 0 && console.log("ondrag");
		*/
	},
	onDragStart: function(){
		...
		// 0 && console.log("onDragStart", e.srcElement.id);
	},
	onDragEnd: function(){
		// summary:
		...
		// 0 && console.log("onDragEnd");
		var e = window.event;
		var node = e.srcElement;
		var id = node.id;
		var doc = this.editor.document;
		// 0 && console.log("NODE:", node.tagName, node.id,  dojo.attr(node, "align"));
		...
		if(node.tagName.toLowerCase()=="table"){
			setTimeout(function(){
				...
				// 0 && console.log("set", node.tagName, dojo.attr(node, "align"))
			}, 100);
		}
	},
	checkAvailable: function(){
		...
		if(this.availableCurrentlySet){
			...
			// plugins get the same return without doing the method over
			// 0 && console.log("availableCurrentlySet:", this.availableCurrentlySet, "currentlyAvailable:", this.currentlyAvailable)
			return this.currentlyAvailable;
		}
		// 0 && console.log("G - checkAvailable...");
		
		if(!this.editor) {
			// 0 && console.log("editor not ready")
			return false;
		}
		if(this.alwaysAvailable) {
			// 0 && console.log(" return always available")
			return true;
		}
...
//		return Math.floor(new Date().getTime() * 0.00000001);
	...
	onKeyDown: function(evt){
		var key = evt.keyCode;
		// 0 && console.log(" -> DOWN:", key);
		if(key == 16){ this.shiftKeyDown = true;}
		if(key == 9) {
			var o = this.getTableInfo();
			// 0 && console.log("TAB ", o.tdIndex, o);
	...
	onDisplayChanged: function(){
		// 0 && console.log("onDisplayChanged")
...
var TablePlugins = declare("dojox.editor.plugins.TablePlugins", _Plugin, {
		...
		onDisplayChanged: function(withinTable){
			...
			// 0 && console.log("onDisplayChanged", this.name);
			if(!this.alwaysAvailable){
			...
			switch(sw){
				...
				case "deleteTableRow":
					o.tbl.deleteRow(o.trIndex);
					 0 && console.log("TableInfo:", this.getTableInfo());
					break;
		...
		begEdit: function(){
			if(this.editor._tablePluginHandler.undoEnabled){
				// 0 && console.log("UNDO:", this.editor.customUndo);
				if(this.editor.customUndo){
					this.editor.beginEditing();
				}else{
					this.valBeforeUndo = this.editor.getValue();
					// 0 && console.log("VAL:", this.valBeforeUndo);
...
var ColorTableCell = declare("dojox.editor.plugins.ColorTableCell", TablePlugins, {
	...
	modTable: function(cmd, args){
		...
		// 0 && console.debug("SELECTED CELLS ", tds , " FOR ", o);
