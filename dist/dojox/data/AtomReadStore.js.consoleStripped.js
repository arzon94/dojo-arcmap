...
var AtomReadStore = dojo.declare("dojox.data.AtomReadStore", null, {
	...
	_getItems: function(document, request){
		...
		if(document.childNodes.length < 1){
			this._items = items;
			 0 && console.log("dojox.data.AtomReadStore: Received an invalid Atom document. Check the content type header");
			return items;
		}
		...
		if(!feedNodes || feedNodes.length != 1){
			 0 && console.log("dojox.data.AtomReadStore: Received an invalid Atom document, number of feed tags = " + (feedNodes? feedNodes.length : 0));
			return items;
		}
